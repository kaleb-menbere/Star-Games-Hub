...
class GameService {
    ...
    getGameImageUrl(gameId, imageType) {
        return `/api/games/image/${gameId}/${imageType}`;
    }
    ...
    // Format game for display (UPDATED for BLOB storage)
    formatGameForDisplay(game) {
        return {
            ...game,
            // Use imageType instead of filename
            bannerUrl: game.hasBanner ? this.getGameImageUrl(game.gameId, 'banner') : null,
            logoUrl: game.hasLogo ? this.getGameImageUrl(game.gameId, 'logo') : null,
            screenshotUrls: game.screenshotUrls || [],
            playUrl: this.getGamePlayUrl(game.gameId),
            formattedPlayCount: this.formatPlayCount(game.playCount || 0)
        };
    }
