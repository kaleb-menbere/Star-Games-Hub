...
void main() {
   ...
   //gl_FragColor = vec4(texel.rgb, texel.a);
    ...
    //vec2 iResolution = vec2(1411.0, 937.0);
	vec2 p=(gl_FragCoord.xy*2.-iResolution.xy)/iResolution.x;
    
    vec2 v=vec2(1E3);
    vec2 v2=vec2(1E4);
    vec2 center=vec2(0.25,0.1);
    for(int c=0;c<30;c++)
    {
...
//    if(length(v-p)<4E-3)col=1E-6;
    ...
    float distortOffset = 0.02;
    vec4 tex= texture2D(uColorBuffer,(gl_FragCoord.xy)/iResolution.xy+rnd(v)*distortOffset);
